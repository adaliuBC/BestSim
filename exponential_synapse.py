...
class exponential:
    ...
    '''
    def __init__(self, g_max = 1., tau = 0.):
        self.g_max = g_max
        self.tau = tau

    def simulate(self, t, s, g, spike_pre, ts):
        dsdt = - s / self.tau
        s = s + ts * dsdt
        if spike_pre:
            s += 1
        stim_post = self.g_max * s
        t += ts
        return t, s, self.g_max * s, stim_post
...
if __name__ == "__main__":
    ...
    stim_pre_list = const_input(16., simu_time, ts)
    ...
    syn = exponential(g_max = g_max, tau = tau_syn)
    ...
    for epoch in range(epochs):
        ...
        _, s_syn_new, g_syn_new, stim_post = syn.simulate(
            t, s_syn, g_syn, spike_pre, ts
        )
        stim_post = stim_post_base_list[epoch] + stim_post  # post neu input = I_ext + I_syn
    ...
    axs[0][1].plot(t_list, s_syn_list, label = "g_syn")
    axs[1][1].plot(t_list, g_syn_list, label = 'V_post')
    axs[2][1].plot(t_list, V_post_list, label = 'stim_post')
    plt.legend()
    plt.show()
